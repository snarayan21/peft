...
class LoraLayer(BaseTunerLayer):
    ...
    def __init__(self, base_layer: nn.Module, ephemeral_gpu_offload: bool = False, **kwargs) -> None:
        ...
        if isinstance(base_layer, nn.Linear):
            ...
            in_features, out_features = (
                base_layer.weight.ds_shape if hasattr(
                    base_layer.weight, "ds_shape") else base_layer.weight.shape
            )
    ...
    def update_layer(
        ...
        self.lora_B[adapter_name] = nn.Linear(
            r, self.out_features, bias=lora_bias)
    ...
    def olora_init(self, adapter_name):
        ...
        if bnb_param_type:
            ...
            raise TypeError(
                f"Unsupported data type for the base layer. Got {dtype}.")
        ...
        weight_tensor.data -= scale_factor * \
    ...
    def pissa_init(self, adapter_name, init_lora_weights):
        ...
        if init_lora_weights == "pissa":
            ...
            Vr, Sr, Ur = svd_lowrank(
                weight.data, self.r[adapter_name], niter=int(
                    init_lora_weights.split("_niter_")[-1])
            )
            Sr /= self.scaling[adapter_name]
            Uhr = Ur.t()
        else:
        ...
        print("lora A dtype", lora_A.dtype)
        print("------")
        print("lora B shape", lora_B.shape)
        print("lora B device", lora_B.device)
        print("lora B dtype", lora_B.dtype)
        print("------")
        print("self.lora_A[adapter_name].weight shape",
              self.lora_A[adapter_name].weight.shape)
        print("self.lora_A[adapter_name].weight device",
              self.lora_A[adapter_name].weight.device)
        print("self.lora_A[adapter_name].weight dtype",
              self.lora_A[adapter_name].weight.dtype)
        print("------")
        print("self.lora_B[adapter_name].weight shape",
              self.lora_B[adapter_name].weight.shape)
        print("self.lora_B[adapter_name].weight device",
              self.lora_B[adapter_name].weight.device)
        print("self.lora_B[adapter_name].weight dtype",
    ...
    def _check_forward_args(self, x, *args, **kwargs):
        ...
        unique_adapters = {
            name for name in adapter_names if name != "__base__"}
        ...
        while len(self.merged_adapters) > 0:
            ...
            if active_adapter in self.lora_A.keys():
                ...
                if not self.use_dora[active_adapter]:
                    ...
                    weight_norm = self._cache_pop(
                        f"{active_adapter}-weight_norm")
                    dora_factor = self.lora_magnitude_vector[active_adapter].weight / weight_norm
                    weight_orig = weight.data / \
                        dora_factor.view(-1, 1) - delta_weight
...
class Embedding(nn.Module, LoraLayer):
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        ...
        output_tensor = transpose(
            weight_B @ weight_A, True) * self.scaling[adapter]
...
class _ConvNd(nn.Module, LoraLayer):
    ...
    def dora_init(self, adapter_name: str) -> None:
        ...
        lora_B = self.lora_B[adapter_name].weight
        scaling = self.scaling[adapter_name]
        dora_layer.update_layer(base_layer=self.get_base_layer(
                ...
                if safe_merge:
                    ...
                    if not self.use_dora[active_adapter]:
                        ...
                        orig_weights = dora_factor.view(
                            *self._get_dora_factor_view()) * (orig_weights + delta_weight)
        ...
        while len(self.merged_adapters) > 0:
            ...
            if active_adapter in self.lora_A.keys():
                ...
                if not self.use_dora[active_adapter]:
                    ...
                    weight_orig = weight.data / \
                        dora_factor.view(
                            *self._get_dora_factor_view()) - delta_weight
                    weight.data = weight_orig
